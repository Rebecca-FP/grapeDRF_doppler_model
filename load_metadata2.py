...
def load_grape_drf_metadata(data_dir,meta_subdir):
    # METADATA LOADING #########################

    meta_dir    = os.path.join(data_dir, 'ch0', meta_subdir)
    ...
    s0, s1      = do.get_bounds(rf_channel)
...
# Read method just reads for first time
    if 'callsign' in fields:
      ...
      theCallsign=rf_channel.partition('_')[2] 
    ...
    properties  = do.get_properties(rf_channel)
