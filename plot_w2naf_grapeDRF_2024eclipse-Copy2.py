# Modified version of plot_w2naf_grapeDRF_2024eclipse.py
# takes command line arguments
# outputs .csv file containing timestamps and doppler shifts 
...
# Empty lists for timestamps and doppler shift
timestamps = []
freq_devs = []

# Set base directory and create paths to data directory & output directory
base_directory='./'
...
hour_start=int(sys.argv[3])  # Start time
hour_end=int(sys.argv[4])    # End time

# Check that start time is between 0 and 23
...
if (float(hour_end)-float(hour_start)) <1:
...
station_dct = {}
sdct    = station_dct[f'{station_name}'] = {}
...
if __name__ == '__main__':
    ...
    sDate       = datetime.datetime(2024,5,10) # want to read this from the metadata
    eDate       = datetime.datetime(2024,5,11) # want to read this from the metadata
    station     = station_name.lower()
    print(station)
    lat         =  41.335116 # W2NAF          # want to read this from the metadata
    lon         = -75.600692 # W2NAF          # want to read this from the metadata
    ...
    if 'WDgrape' in plot_list:
        ...
        for cfreq in cfreqs:
            ...
            if os.path.exists(rt_dop_csv):
                ...
                # Save timestamps and fd to .csv file
                xx = rt_dop['time']
                yy = rt_dop['fd']
                xx_list = xx.tolist()
                yy_list = yy.tolist()
                ...
                data_frame.to_csv('time_vs_fd',index=False)
